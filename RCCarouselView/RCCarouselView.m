...
- (void)__configData {
    if (self.delegate && [self.delegate respondsToSelector:@selector(numberOfItemsInCarouselView)] && [self.delegate respondsToSelector:@selector(carouselView:itemForRowAtIndex:)]) {
        ...
        GHLogSystem(@"__configData delegate not exist");
    }
}

- (void)refreshData {
    [self.itemArray removeAllObjects];
    [self.mainView.layer.sublayers makeObjectsPerformSelector:@selector(removeFromSuperlayer)];
    [self __configData];
}
...
- (void)setCurrentIndex:(int)currentIndex {
    ...
    GHLogDebug(@"setCurrentIndex:%d", currentIndex);
...
- (void)__moveToIndex:(int)index {
    ...
    GHLogDebug(@"__moveToIndex offset:%lf--offsetItems:%lf", offsetToAdd, _leftMostItem.x);
    [self moveCarousel:offsetToAdd];
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(carouselView:willDisplayItemforIndex:)]) {
        [self.delegate carouselView:self willDisplayItemforIndex:_currentIndex];
    }
}
...
- (void)panGestureAction:(UIPanGestureRecognizer *)pan {
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            GHLogDebug(@" xOffset:%lf, _endGesturePoint:%lf=%lf", xOffset, _endGesturePoint.x, _endGesturePoint.y);
